...
def main():
    ...
    infile.close()
    outfile = open("hw13project1.txt", "a")
    
    start_time = time.time_ns()
    mergeSort(nums)
    finish_time = time.time_ns()

    print("Merge sort of {} numbers".format(len(nums)), file=outfile)
    print("Start time: {}".format(start_time), file=outfile)
    print("Finish time: {}".format(finish_time), file=outfile)
    print("Total time: {} seconds".format((finish_time-start_time)/10**9), file=outfile)
